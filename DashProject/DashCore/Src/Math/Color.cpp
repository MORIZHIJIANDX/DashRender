...
namespace Dash
{
	...
	*/
	...
	{
		const float	Primary = FMath::Max(r, g, b);
		FColor	Color;
	...
	{
		size_t StartIndex = (!HexString.empty() && HexString[0] == TCHAR('#')) ? 1 : 0;
		...
		if (HexString.length() == (6 + StartIndex))
		{
			...
			Result.r = (HexDigit(HexString[StartIndex + 0]) << 4) + HexDigit(HexString[StartIndex + 1]);
			Result.g = (HexDigit(HexString[StartIndex + 2]) << 4) + HexDigit(HexString[StartIndex + 3]);
			Result.b = (HexDigit(HexString[StartIndex + 4]) << 4) + HexDigit(HexString[StartIndex + 5]);
			Result.a = 255;
		...
		if (HexString.length() == (8 + StartIndex))
		{
			...
			Result.g = (HexDigit(HexString[StartIndex + 2]) << 4) + HexDigit(HexString[StartIndex + 3]);
			Result.b = (HexDigit(HexString[StartIndex + 4]) << 4) + HexDigit(HexString[StartIndex + 5]);
			Result.a = (HexDigit(HexString[StartIndex + 6]) << 4) + HexDigit(HexString[StartIndex + 7]);
	...
	FLinearColor FColor::FromRGBE() const
	{
		...
		{
			const float Scale = FMath::Ldexp(1 / 255.0f, a - 128);
	...
	//	const TScalarArray<float, 3> Hue = (H < 86) ? TScalarArray<float, 3>((85 - H) / 85.f, (H - 0) / 85.f, 0.0f) : (H < 171) ? TScalarArray<float, 3>(0.0f, (170 - H) / 85.f, (H - 85) / 85.f) : TScalarArray<float, 3>((H - 170) / 85.f, 0.0f, (255 - H) / 84.f);
	//	const TScalarArray<float, 3> ColorVector = (Hue + S / 255.f * (TScalarArray<float, 3>(1, 1, 1) - Hue)) * Brightness;
	...
	{
		const float RGBMin = FMath::Min(r, g, b);
		const float RGBMax = FMath::Max(r, g, b);
	...
	*/
	...
	{
		float MaxComponent = FMath::Max(TScalarTraits<float>::Epsilon(), FMath::Max(InLinearColor.r, FMath::Max(InLinearColor.g, InLinearColor.b)));
	...
	float FLinearColor::Pow22OneOver255Table[256] =
	{
		...
		0.948964938178195f, 0.957369576199527f, 0.96581465350313f, 0.974300202388861f, 0.982826255053791f, 0.99139284359294f, 1
	};
	...
	*
	* Color > 0.04045 ? pow( Color * (1.0 / 1.055) + 0.0521327f, 2.4 ) : Color * (1.0 / 12.92);
	*/
